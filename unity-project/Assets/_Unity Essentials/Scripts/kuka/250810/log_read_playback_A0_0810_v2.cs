...
public class A0LogReplayer0810 : MonoBehaviour
{
    /*────────────[ Inspector ]────────────*/
    [Header("Log File")]
    ...
    [Tooltip("ستون 1: مقادیر لحظه‌ای (سریال/لاین‌های جدید لاگ)")]
    public TMP_Text liveDataLabel;

    [Tooltip("ستون 2: مقادیر حالت آفلاین (اسکرول/پلی‌بک)")]
    public TMP_Text offlineDataLabel;

    [Tooltip("ریشهٔ بصری ستون آفلاین (Panel یا GameObject حاوی offlineDataLabel). در Live پنهان می‌شود.")]
    public GameObject offlineColumnRoot;
    ...
    /*────────────[ کشِ جداگانه برای هر ستون ]────────────*/
    // ستون Live (از تهِ لاگ/سریال)
    private readonly float[] liveA = new float[7]; // A1..A6, A0
    private int liveFlag = 0;
    private bool hasLive = false;

    // ستون Offline (انتخاب/پلی‌بک)
    private readonly float[] offA = new float[7];  // A1..A6, A0
    private int offFlag = 0;
    private bool hasOff = false;
    ...
    private void Start()
    {
        ...
        // ستون آفلاین در شروع پنهان
        if (offlineColumnRoot) offlineColumnRoot.SetActive(false);

        tailCo = StartCoroutine(Tail());
        ShowStatus("Live");

        // پاکسازی اولیهٔ ستون‌ها
        UpdateLiveUI();
        UpdateOfflineUI();
    }
    ...
    private IEnumerator Tail()
    {
        ...
        while (true)
        {
            ...
            if (TryParseAll(line, out float[] a, out int flag))
            {
                // همیشه ستون Live را تازه‌کن
                CacheLiveAndUpdateUI(a, flag);

                // فقط وقتی لایو هستیم، موشن را از Live بگیر
                if (mode == Mode.Live)
                {
    ...
    private void EnterOffline()
    {
        ...
        if (offlineColumnRoot) offlineColumnRoot.SetActive(true);
        ShowStatus($"Offline  ({baseDiscard + offsets.Count:n0} lines)");
        ApplySelection(); // نمایش فوری ستون آفلاین
    }
    ...
    private void ApplySelection()
    {
        ...
        {
            CacheOfflineAndUpdateUI(a, flag);
            targetX = a[6] * a_scale + b_offset; // موشن از ستون آفلاین
            firstValue = true;
        }
    }
    ...
    /*───────── UI helpers: Live / Offline ─────────*/
    private void CacheLiveAndUpdateUI(float[] a, int flag)
    {
        if (a == null || a.Length < 7) return;
        for (int i = 0; i < 7; i++) liveA[i] = a[i];
        liveFlag = flag;
        hasLive = true;
        UpdateLiveUI();
    }

    private void CacheOfflineAndUpdateUI(float[] a, int flag)
    {
        if (a == null || a.Length < 7) return;
        for (int i = 0; i < 7; i++) offA[i] = a[i];
        offFlag = flag;
        hasOff = true;
        UpdateOfflineUI();
    }
    ...
    private void UpdateLiveUI()
    {
        ...
        liveDataLabel.text =
            $"Current Value \n" +
            $"A1: {liveA[0].ToString("0.#####", cul)}\n" +
            $"A2: {liveA[1].ToString("0.#####", cul)}\n" +
            $"A3: {liveA[2].ToString("0.#####", cul)}\n" +
            $"A4: {liveA[3].ToString("0.#####", cul)}\n" +
            $"A5: {liveA[4].ToString("0.#####", cul)}\n" +
            $"A6: {liveA[5].ToString("0.#####", cul)}\n" +
            $"A0: {liveA[6].ToString("0.#####", cul)}\n" +
            $"Spray: {liveFlag}";
    }

    private void UpdateOfflineUI()
    {
        if (!offlineDataLabel) return;

        if (!hasOff)
        {
            offlineDataLabel.text =
                "Offline Value \nA1: –\nA2: –\nA3: –\nA4: –\nA5: –\nA6: –\nA0: –\nSpray: –";
            return;
        }
        ...
        offlineDataLabel.text =
            $"Offline Value \n" +
            $"A1: {offA[0].ToString("0.#####", cul)}\n" +
            $"A2: {offA[1].ToString("0.#####", cul)}\n" +
            $"A3: {offA[2].ToString("0.#####", cul)}\n" +
            $"A4: {offA[3].ToString("0.#####", cul)}\n" +
            $"A5: {offA[4].ToString("0.#####", cul)}\n" +
            $"A6: {offA[5].ToString("0.#####", cul)}\n" +
            $"A0: {offA[6].ToString("0.#####", cul)}\n" +
            $"Spray: {offFlag}";
    }
