// ====================================================================
//  A1LogRotator.cs  –  نسخهٔ اصلاح‌شده بدون پرش و بدون تغییر Scale
//  خواندن مقدار A1 از لاگ و تنظیم Rotation Y
//      yu = a * yr + b
// ====================================================================
...
public class A1LogRotator0805 : MonoBehaviour
{
    /*──────── تنظیمات ────────*/
    [Header("Log File")]
    ...
    [Header("A1  →  Rotation Y")]
    public float a_scale = 0.1f;               // ضریب تبدیل
    public float b_offset = 0f;                // افست (درجه)

    /*──────── وضعیت داخلی ────────*/
    private float currentAngleY;               // زاویهٔ هموارشدهٔ داخلی برای Y
    private float angleVelocity = 0f;
    private volatile float targetAngleY;
    private bool firstValue = false;

    // محورهای اولیهٔ X و Z را حفظ می‌کنیم
    ...
    private void Start()
    {
        // زاویه‌های اولیهٔ محلی
        Vector3 init = transform.localEulerAngles;
        initialLocalX = init.x;
        currentAngleY = init.y;    // مقدار ابتدایی محور Y
    ...
    private IEnumerator Tail()
    {
        ...
        while (true)
        {
            ...
            if (mode == Mode.Live && TryParseA1(line, out float a1))
            {
                a1 = a1 * a_scale + b_offset;
                targetAngleY = a1;
                firstValue = true;
            }
    ...
    {
        if (TryReadA1(selectorIdx, out float a1))
        {
            a1 = a1 * a_scale + b_offset;
            targetAngleY = a1;
            firstValue = true;
    ...
    private IEnumerator Playback()
    {
        ...
        {
            if (TryReadA1(playIdx, out float a1))
            {
                a1 = a1 * a_scale + b_offset;
                targetAngleY = a1;
                firstValue = true;
    ...
    private void ReturnLive()
    {
        ...
        if (lastIdx >= 0 && TryReadA1(lastIdx, out float a1))
        {
            a1 = a1 * a_scale + b_offset;
            targetAngleY = a1;
        }
    ...
    /*───────── Helper: read A1 ─────────*/
    private bool TryReadA1(int idx, out float a1)
    {
        a1 = default;
        ...
        using var sr = new StreamReader(fs);
        string line = sr.ReadLine();
        return TryParseA1(line, out a1);
    }

    /*───────── Parse line ─────────*/
    private static bool TryParseA1(string raw, out float a1)
    {
        a1 = default;
        ...
        return tok.Length > 0 &&                        // A1 اولین مقدار
               float.TryParse(tok[0], NumberStyles.Float,
                              CultureInfo.InvariantCulture, out a1);
    }
}
