// ====================================================================
//  A2LogRotator.cs  –  نسخهٔ اصلاح‌شده بدون پرش و بدون تغییر Scale
//  خواندن مقدار A2 از لاگ و تنظیم Rotation X
//      xu = a * xr + b
// ====================================================================
...
public class A2LogRotator0805 : MonoBehaviour
{
    /*──────── تنظیمات ────────*/
    [Header("Log File")]
    ...
    [Header("A2  →  Rotation X")]
    public float a_scale = 0.1f;                     // مثلاً °/10
    public float b_offset = 0f;                      // افست پایه (درجه)
    ...
    // محورهای اولیهٔ Y و Z را ثابت نگه می‌داریم
    ...
    private void Start()
    {
        // زاویه‌های اولیهٔ محلی را ذخیره می‌کنیم
    ...
    private void Update()
    {
        ...
        // حفظ Y و Z اولیه و نوشتن یکبارهٔ کواترنیون
    ...
    private IEnumerator Tail()
    {
        ...
        while (true)
        {
            ...
            if (mode == Mode.Live && TryParseA2(line, out float a2))
            {
                targetAngleX = a2 * a_scale + b_offset;
                firstValue = true;
            }
    ...
    {
        if (TryReadA2(selectorIdx, out float a2))
        {
            targetAngleX = a2 * a_scale + b_offset;
            firstValue = true;
        }
    ...
    private IEnumerator Playback()
    {
        ...
        {
            if (TryReadA2(playIdx, out float a2))
            {
                targetAngleX = a2 * a_scale + b_offset;
                firstValue = true;
            }
    ...
    private void ReturnLive()
    {
        ...
        if (lastIdx >= 0 && TryReadA2(lastIdx, out float a2))
        {
            targetAngleX = a2 * a_scale + b_offset;
        }
    }
    ...
    /*──────── Helpers ────────────*/
    private bool TryReadA2(int idx, out float a2)
    {
        a2 = default;
        ...
        using var sr = new StreamReader(fs);
        string line = sr.ReadLine();
        return TryParseA2(line, out a2);
    }

    private static bool TryParseA2(string raw, out float a2)
    {
        a2 = default;
        ...
        return tok.Length > 1 &&
               float.TryParse(tok[1], NumberStyles.Float,
                              CultureInfo.InvariantCulture, out a2);
    }
}
